...
async function cleanProductionDatabase() {
  ...
  try {
    console.log(
      "⚠️  DANGER: This will DROP ALL TABLES and DELETE ALL DATA in PRODUCTION database!",
    );
    ...
    if (!process.env.CONFIRM_PRODUCTION_CLEAN) {
      ...
      console.error(
        "💡 Set CONFIRM_PRODUCTION_CLEAN=true to confirm production database destruction",
      );
      process.exit(1);
    }
